...
PLACE NAME RULES (very important):
- Chinese-speaking regions (Taiwan, Hong Kong, Macau, mainland China): Use Chinese name as primary. If it also has an English name, add it after. e.g. "小騙徒潑畫體驗 Paint Baby Studio"
- Korea: Use Korean name as primary. Add Chinese translation in parentheses if commonly known. e.g. "Planer House (플레이너하우스)"
- Japan: Use Japanese name as primary. Add Chinese translation if commonly known.
- Other regions: Use original local name as primary. Add Chinese name in parentheses if it exists. e.g. "Eiffel Tower（艾菲爾鐵塔）"
- City/country: Always use Traditional Chinese. e.g. 高雄、台灣、首爾、韓國、東京、日本、巴黎、法國

JSON fields:
- placeName: formatted name following above rules
- city: Traditional Chinese city name
- country: Traditional Chinese country name

CRITICAL: Always include the city/region in search queries.
- Use context clues to find the city/region (location tags, hashtags, text mentioning city name).
- For Korean places use Korean city names in naverQuery: 제주도, 서울, 부산, etc.
- naverQuery format: "place name + city in Korean" (only for Korean places)
- googleQuery format: "place name + city in English"

Respond ONLY with valid JSON array, no markdown:
[
  {"placeName":"formatted name per rules above","city":"Traditional Chinese city","country":"Traditional Chinese country","naverQuery":"place + city in Korean","googleQuery":"place name + city in English","confidence":"high"},
  ...
]
...
CRITICAL: Always include the city/region in search queries.
- If the post mentions a city or region, include it in EVERY place's query.
- For Korean places use Korean city names in naverQuery: 제주도, 서울, 부산, etc.
- naverQuery format: "place name + city in Korean" (only for Korean places)
- googleQuery format: "place name + city in English"
...
function buildLocationMessage(place, index, total) {
  ...
  const isKorea = ['한국', 'korea', 'south korea', '韓國', '韓国', '한국'].some(k => countryLower.includes(k));
